...
var initial = true; // Override Settings
...
var bcSfFilterSettings = {
  general: {
    ...
    activeFilterScrollbarPC: false,
    paginationType: bcSfFilterConfig.custom.pagination_type.indexOf("load_more") != -1 ? "load_more" : bcSfFilterConfig.custom.pagination_type == 'infinite_scroll' ? 'infinite' : 'default'
  }
}; // Declare Templates

var bcSfFilterTemplate = {
  // Grid Template
  'productGridItemHtml': '<div class="{{itemDesktopWidthClass}} medium-down--one-half {{itemMobileWidthClass}} column {{itemQuickShopClass}} {{itemSwapImageClass}} {{itemStickersClass}}' + ' thumbnail product__thumbnail product__grid-item' + ' thumbnail__hover-overlay--{{itemHoverClass}}' + ' has-padding-bottom">' + '<div class="product-wrap">' + '<div class="product-image__wrapper">' + '<div class="image__container product__imageContainer">' + '<a href="{{itemUrl}}">' + '{{itemStickers}}' + '{{itemImages}}' + '{{itemHover}}' + '</a>' + '</div>' + '</div>' + '<div class="thumbnail__caption text-align-{{itemTextAlignClass}}">' + '{{itemProductInfo}}' + '</div>' + '</div>' + '{{itemSwatch}}' + '{{itemReviews}}' + '</div>',
  'itemHoverHtml': '<div class="thumbnail-overlay__container">' + '<a class="hidden-product-link" href="{{itemUrl}}">{{itemTitle}}</a>' + '{{itemProductInfoHover}}' + '{{itemQuickShop}}' + '{{itemReviewsHover}}' + '</div>',
  'itemProductInfoHtml': '<div class="product-thumbnail">' + '{{itemVendor}}' + '<a href="{{itemUrl}}" class="product-thumbnail__title">{{itemTitle}}</a>' + '{{itemComingSoon}}' + '{{itemPrice}}' + '</div>',
  // Pagination Template
  ...
  // Sorting Template
  'sortingHtml': '<select name="sources" aria-label="Sort by" id="sources" class="sort_by custom-select sources" placeholder="Sort By">{{sortingItems}}</select>'
}; // Build Product Grid Item

BCSfFilter.prototype.buildProductGridItem = function (data) {
  // Get Template
  var itemHtml = bcSfFilterTemplate.productGridItemHtml; // Add class

  var itemDesktopWidthClass = 'one-third';

  switch (bcSfFilterConfig.custom.products_per_row) {
    case 1:
      itemDesktopWidthClass = 'one-whole';
      break;

    case 2:
      itemDesktopWidthClass = 'one-half';
      break;

    case 3:
      itemDesktopWidthClass = 'one-third';
      break;

    case 4:
      itemDesktopWidthClass = 'one-fourth';
      break;

    case 5:
      itemDesktopWidthClass = 'one-fifth';
      break;

    case 6:
      itemDesktopWidthClass = 'one-sixth';
      break;

    case 7:
      itemDesktopWidthClass = 'one-seventh';
      break;

    case 8:
      itemDesktopWidthClass = 'one-eighth';
      break;
  }
  ...
  itemHtml = itemHtml.replace(/{{itemSwapImageClass}}/g, bcSfFilterConfig.custom.show_secondary_image && data.images_info.length > 1 ? 'has-secondary-image-swap' : '');
  itemHtml = itemHtml.replace(/{{itemHoverImage}}/g, bcSfFilterConfig.custom.show_secondary_image && data.images_info.length > 1 ? 'has-secondary-image-swap' : '');
  itemHtml = itemHtml.replace(/{{itemTextAlignClass}}/g, bcSfFilterConfig.custom.thumbnail_text_alignment); // Add stickers

  var itemStickers = buildStickers(data);
  itemHtml = itemHtml.replace(/{{itemStickers}}/g, itemStickers); // Add images

  var itemImages = buildImages(data);
  itemHtml = itemHtml.replace(/{{itemImages}}/g, itemImages); // Add hover elements

  var itemHover = buildHover(data);
  itemHtml = itemHtml.replace(/{{itemHover}}/g, itemHover); // Add product info

  itemHtml = itemHtml.replace(/{{itemProductInfo}}/g, bcSfFilterTemplate.itemProductInfoHtml); // Add vendor
  ...
  var collectionHandles = data.collections.map(function (x) {
    return x.handle;
  });
  var isComingSoon = collectionHandles.indexOf('coming-soon') != -1; // Add coming soon

  if (isComingSoon) {
    itemHtml = itemHtml.replace(/{{itemComingSoon}}/g, '<span>' + bcSfFilterConfig.label.coming_soon + '</span>');
    itemHtml = itemHtml.replace(/{{itemPrice}}/g, ''); // Add price
  } else {
  ...
  itemHtml = itemHtml.replace(/{{itemSwatch}}/g, itemSwatch); // Add reviews

  var itemMoreCol = buildMoreColor(data);
  itemHtml = itemHtml.replace(/{{itemSwatch}}/g, itemMoreCol);
  ...
  var itemQuickShop = bcSfFilterConfig.custom.enable_quickshop ? '<div data-bc-sf-quickshop-id="{{itemId}}" style="opacity:0;"></div>' : '';
  itemHtml = itemHtml.replace(/{{itemQuickShop}}/g, itemQuickShop); // Add main attribute (Always put at the end of this function)
...
function buildStickers(data) {
  ...
  if (bcSfFilterConfig.custom.stickers_enabled) {
    var collections = data.collections.map(function (x) {
      return x.handle;
    });
    var stickerCollections = ['best-sellers', 'coming-soon', 'new', 'pre-order', 'staff-pick', 'ak-closet', 'considered', 'exclusive'];
    ...
    if (stickers.length > 0) {
      html = '<div class="sticker-holder sticker-shape-' + bcSfFilterConfig.custom.sticker_shape + ' sticker-position-' + bcSfFilterConfig.custom.sticker_position + '">' + '<div class="sticker-holder__content sticker-holder__content--">' + stickers.join('') + '</div>' + '</div>';
    }
  }
...
function buildImageElement(image, additionalClass) {
  ...
  html += '<div class="image-element__wrap" style="' + backgroundColor + heightStyle + widthStyle + '">' + '<img alt="{{itemTitle}}"' + ' class="lazyload transition--' + bcSfFilterConfig.custom.image_loading_style + ' ' + additionalClass + '"' + (lowQualityImage ? ' src="' + lowQualityImage + '"' : '') + ' data-src="' + bcsffilter.optimizeImage(image.src, '1600x') + '"' + ' data-sizes="auto"' + ' data-aspectratio="' + aspectRatio + '"' + ' data-srcset="' + bgset(image) + '"' + ' height="' + image.height + '"' + ' width="' + image.width + '"' + ' style="' + heightStyle + '"' + '/>' + '</div>';
  return html;
}
...
function buildHover(data) {
  ...
  if (bcSfFilterConfig.custom.enable_quickshop || bcSfFilterConfig.custom.thumbnail_hover_enabled) {
    ...
    if (bcSfFilterConfig.custom.thumbnail_hover_enabled) {
      var itemProductInfoHover = '<div class="quick-shop__info">' + '<div class="thumbnail-overlay">' + '<div class="info text-align-center">' + '{{itemProductInfo}}' + '</div>' + '</div>' + '</div>';
...
function buildPrice(data) {
  ...
  if (!soldOut) {
    ...
    if (onSale) {
      html += '<span class="product-thumbnail__was-price was-price">' + '<span class="money">' + bcsffilter.formatMoney(data.compare_at_price_max) + '</span>' + '</span>';
    }
  } else {
...
function buildSwatch(data) {
  ...
  if (bcSfFilterConfig.custom.collection_swatches) {
    data.variants.forEach(function (variant) {
      ...
      if (colorValue) {
        var swatchItem = '<a href="{{variantUrl}}" class="swatch swatch__style--{{swatchShape}}" data-swatch-name="meta-{{slugifyOptionValue}}_{{slugifyColorValue}}">' + '<span data-image="{{variantImage}}" style="background-color: {{colorBackground}};">' + '<img class="swatch__image" src="{{colorImage}}" onerror="this.className += \' swatch__image--empty\'">' + '</span>' + '</a>';
        ...
        if (variant.image) {
          switch (bcSfFilterConfig.custom.products_per_row) {
            case 2:
              variantImage = bcsffilter.optimizeImage(variant.image, '600x');
              break;

            case 3:
              variantImage = bcsffilter.optimizeImage(variant.image, '500x');
              break;

            default:
              variantImage = bcsffilter.optimizeImage(variant.image, '400x');
              break;
          }
        }
  ...
  if (swatchItems.length > 1) {
    html = '<a href="{{itemUrl}}" class="more_color">+More Colors</a>';
  }

  if ($.inArray("Overflow", data.tags) !== -1) {
    html = '<a href="{{itemUrl}}" class="more_color">+More Colors</a>';
  }
...
BCSfFilter.prototype.buildPagination = function (totalProduct) {
  ...
  var totalPage = Math.ceil(totalProduct / this.queryParams.limit); // If it has only one page, clear Pagination
  ...
  if (this.getSettingValue('general.paginationType') == 'default') {
    var paginationHtml = bcSfFilterTemplate.paginateHtml; // Build Previous

    var previousHtml = currentPage > 1 ? bcSfFilterTemplate.previousActiveHtml : '';
    previousHtml = previousHtml.replace(/{{itemUrl}}/g, this.buildToolbarLink('page', currentPage, currentPage - 1));
    paginationHtml = paginationHtml.replace(/{{previous}}/g, previousHtml); // Build Next

    var nextHtml = currentPage < totalPage ? bcSfFilterTemplate.nextActiveHtml : '';
    nextHtml = nextHtml.replace(/{{itemUrl}}/g, this.buildToolbarLink('page', currentPage, currentPage + 1));
    paginationHtml = paginationHtml.replace(/{{next}}/g, nextHtml); // Create page items array
...
BCSfFilter.prototype.buildFilterSorting = function () {
  if (bcSfFilterTemplate.hasOwnProperty('sortingHtml')) {
    ...
    if (sortingArr) {
      ...
      jQ(this.selector.topSorting).html(html); // sort by
      ...
      if (!initial) {
        jQ(".custom-select-trigger").text(sortingArr[this.queryParams.sort]);
      }
    }
  }
...
BCSfFilter.prototype.buildSortingEvent = function () {
  var self = this;
  jQ(this.getSelector('topSorting') + ' select').change(function (e) {
    onInteractWithToolbar(e, 'sort', self.queryParams.sort, jQ(this).val());
  });
  jQ('.custom-options .custom-option').on('click', function (e) {
    onInteractWithToolbar(e, 'sort', self.queryParams.sort, jQ(this).data('value'));
  });
}; // Add additional feature for product list, used commonly in customizing product list
...
BCSfFilter.prototype.prepareProductData = function (data) {
  ...
  for (var k = 0; k < countData; k++) {
    ...
    var isRoundedPrice = true;

    if (firstVariant.hasOwnProperty("fulfillment_service") && firstVariant.fulfillment_service == "gift_card") {
      isRoundedPrice = false;
    }

    if (typeof self.convertPriceBasedOnActiveCurrency !== "undefined") {
      data[k].price_min = self.convertPriceBasedOnActiveCurrency(data[k].price_min, isRoundedPrice);
      data[k].price_max = self.convertPriceBasedOnActiveCurrency(data[k].price_max, isRoundedPrice);
      data[k].compare_at_price_min = self.convertPriceBasedOnActiveCurrency(data[k].compare_at_price_min, isRoundedPrice);
      data[k].compare_at_price_max = self.convertPriceBasedOnActiveCurrency(data[k].compare_at_price_max, isRoundedPrice);
    }

    data[k]["price_min"] *= 100, data[k]["price_max"] *= 100;

    if (data[k]["compare_at_price_min"] != null) {
    ...
    for (var i = 0; i < countVariants; i++) {
      ...
      if (data[k]["variants"][i]["compare_at_price"] != null) {
        var variantCompareAtPrice = parseFloat(data[k]["variants"][i]["compare_at_price"]);
      ...
      var variantPrice = parseFloat(data[k]["variants"][i]["price"]);
